...
def make_filter_from_face(land_mark,main_img):
    ...
    crop=mask[y:y+h,x-1:x+w+1]
    crop = cv2.resize(crop, (0,0), fx=2, fy=2) 
    mask[y-h//2:y-h//2+crop.shape[0],x-w//2:x-w//2+crop.shape[1]]=crop
...
def put_filter_on_face(foreground,background):
    ...
    lip_binary_blur = cv2.multiply(cv2.subtract(lip_binary_blur, 140), 2)
...
fd = UltraLightFaceDetecion("Assignment-30/4.big-eye-lips-filter/weights/RFB-320.tflite",conf_threshold=0.88)

fa = CoordinateAlignmentModel("Assignment-30/4.big-eye-lips-filter/weights/coor_2d106.tflite")

img=cv2.imread("Assignment-30/4.big-eye-lips-filter/image.jpg")
